...
| I2 corpus chunk run (2026-05-28) | `womblex chunk --shards run-20260525T214943Z/documents/` over the i1b extraction. 11,797 chunks across 2,610 of 2,626 completed docs (16 zero-chunk). _(The "16 zero-chunk = correct behaviour, no text-bearing elements" read here was **wrong** — diagnosed via K9-fig: those 16 are full-page scans whose text was mis-tagged `figure` and excluded from chunking. **Correction (2026-05-31 QA): the K9-fig relabel was never applied to this run — these 11,797/2,610/16 numbers are the live, current state. The projected post-fix ~12,506/2,626/0 requires the pending re-extraction.**)_ 9,933 narrative + 1,864 table chunks; median 3 chunks/doc, mean 4.5, max 948 (FOI master spreadsheet manifest, single source). Median chunk len 1,540 chars; 3 chunks corpus-wide are bare `"2"` page-number footers stranded between full-capacity neighbours (cosmetic). `has_redaction=False` everywhere as expected under per-stage execution (in-memory PII/redact-blackout no longer flows into chunks; P3/P6 sidecars will reattach via their own joins). |
...
- **I2 ✓ landed 2026-05-27 (build) + corpus run completed 2026-05-28 — `womblex chunk` per-stage CLI + chunks sidecar writer.**
  ...
  - Audit: 11,797 chunks across 2,610 of 2,626 completed docs (16 zero-chunk). _(K9-fig diagnosed the 16 as full-page scans mis-tagged `figure`. **Correction (2026-05-31 QA): the relabel was never applied to the live run — 11,797/2,610/16 is current; post-fix ~12,506/2,626/0 awaits the pending re-extraction.**)_ 9,933 narrative + 1,864 table chunks; median 3 chunks/doc, mean 4.5, max 948 (the FOI master spreadsheet manifest). Median chunk len 1,540 chars. `has_redaction=False` everywhere by construction (in-memory PII/redact-blackout mutations no longer flow through to chunks under per-stage execution; tracked in P3/P6 sidecars).
...
| **K9-fig** ⚠️ code landed + committed 2026-05-31; **corpus relabel NOT applied** | 1,056 of 1,200 i1b `kind='figure'` elements are `extractor=ocr_paddle`, `bbox=(0,0,1,1)`, paragraph-text content. **Root cause was NOT `_ocr_image_regions` (the original E4 diagnosis named the wrong site).** It is the dominant-region fallback in `_layout_blocks_and_tables`: YOLO layout regions carry no segmented text, so the fallback always fires and collapses the whole page's OCR onto one block using the *largest* region's kind. When that is DocLayNet `Picture`→`figure`, a full-page scan becomes one `figure` excluded from chunking. Fix: shared helper `_ocr_region_block_type(text, layout_kind)` promotes a non-text fallback kind to `paragraph` when OCR yields ≥5 words; sparse output (page numbers, bare logos) keeps `figure`. Threshold calibrated on the 1,056 affected elements. **Correction (2026-05-31 QA):** the fix is *code only* — it was uncommitted in the working tree until `d1d66d6` and the claimed in-place relabel was **never persisted to the live run** `run-20260525T214943Z`, which still measures `figure=1,200`, `chunks=11,797`, `docs-with-chunks=2,610` (the *pre*-fix numbers). The "Deltas i1b→i1c figure 1,200→154 … chunks 11,797→12,506" below were a sample/working-copy measurement, **not the corpus state**. Verified on re-extracting R-03247 with the committed code: page-0 → `paragraph` (was `figure`). **Recovering the corpus requires re-extraction (or relabel) with the now-committed code — pending.** | [strategies_scanned.py:288](src/womblex/ingest/strategies_scanned.py#L288); fallback [strategies_scanned.py:440](src/womblex/ingest/strategies_scanned.py#L440) |
