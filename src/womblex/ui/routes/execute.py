...
class EnqueueRequest(BaseModel):
    ...
    ``input_prefix`` is store-relative (the enqueue lists ``<store>/<prefix>``),
    matching ``womblex enqueue --input-prefix``. ``run_id`` omitted mints a
    fresh timestamped id; supplying an existing one resumes it, since enqueue
    is idempotent on ``(run_id, batch_num)``.
    """
...
def post_enqueue(
    ...
    403 when the console is audit-only, 409 when no store or queue is
    configured (the two states :class:`ExecutionDisabled` distinguishes), 400
    on bad input (no documents under the prefix, an unsafe run_id).
    """
    try:
