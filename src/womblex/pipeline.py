"""Pipeline orchestration — separated, composable stages.

Extraction runs first.  All subsequent stages are independent and can
...
- ``run_enrichment``    — call Isaacus (requires chunks).

Only enrichment has a dependency on chunking.  Redaction, PII cleaning,
and chunking operate on extraction output and do not depend on each other.

``run_pipeline`` runs all enabled stages.  ``process_file`` and
``process_batch`` are thin wrappers for backward compatibility.
"""
...
from womblex.config import PipelineConfig
...
def run_extraction(paths: list[Path], config: PipelineConfig) -> list[DocumentResult]:
...
def run_redaction(
    results: list[DocumentResult], config: PipelineConfig
) -> list[DocumentResult]:
...
def run_chunking(
    ...
    for dr in results:
        ...
        tables = dr.extraction.tables if config.chunking.chunk_tables else None
        dr.chunks = chunk_document(dr.extraction.full_text, chunker, tables=tables)

        # Flag-mode redaction annotation on chunks
        redaction_report = getattr(dr.extraction, "redaction_report", None)
        if (
...
def run_pipeline(paths: list[Path], config: PipelineConfig) -> BatchResult:
    """Run all enabled pipeline stages.

    Extraction always runs first.  Redaction, PII cleaning, and chunking
    are independent post-extraction stages — they operate on extraction
    output and do not depend on each other.  Enrichment requires chunks
    and an Isaacus client, so it is not run here; call ``run_enrichment``
    separately.
    ...
    Returns:
        BatchResult with per-unit results.
    """
    results = run_extraction(paths, config)

    # Post-extraction stages are independent — order here is arbitrary.
    ...
    logger.info(
        "Pipeline complete: %d units, %d succeeded, %d failed",
        len(results),
        batch.succeeded,
        batch.failed,
    )
...
def process_file(path: Path, config: PipelineConfig) -> list[DocumentResult]:
    """Process a single document through all enabled pipeline stages."""
    return run_pipeline([path], config).results


def process_batch(paths: list[Path], config: PipelineConfig) -> BatchResult:
    """Process a batch of documents through all enabled pipeline stages."""
    batch = run_pipeline(paths, config)
    logger.info(
        "Processed batch: units=%d succeeded=%d failed=%d",
        len(batch.results),
        batch.succeeded,
        batch.failed,
    )
...
# ---------------------------------------------------------------------------
# Enrichment batch wrapper (backward compat)
# ---------------------------------------------------------------------------
