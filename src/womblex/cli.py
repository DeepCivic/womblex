...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    from womblex.pipeline import process_batch, write_batch_parquet
    ...
    for batch_num, i in enumerate(range(0, total_files, batch_size), start=1):
        ...
        batch = process_batch(batch_files, config)
...
def cmd_extract(args: argparse.Namespace) -> int:
    ...
    from womblex.pipeline import run_extraction
    ...
    for result in results:
        if result.error:
            logger.error("  %s: %s", result.doc_id, result.error)
        else:
            text = result.extraction.full_text if result.extraction else ""
            out_path = output_dir / f"{result.doc_id}.txt"
            out_path.write_text(text, encoding="utf-8")
            logger.info("  %s -> %s (%d chars)", result.doc_id, out_path, len(text))
...
def cmd_chunk(args: argparse.Namespace) -> int:
    ...
    from womblex.config import load_config
    from womblex.pipeline import run_chunking, run_extraction, write_batch_parquet, BatchResult
...
def cmd_redact(args: argparse.Namespace) -> int:
    ...
    from womblex.config import load_config
    from womblex.pipeline import run_extraction, run_redaction, write_batch_parquet, BatchResult
    ...
    redacted_count = sum(
        ...
        and r.extraction
        and getattr(r.extraction, "redaction_report", None)
