...
| `process/money.py` | Self-evidencing money recognition (`find_money`): the `docs/money.md` pattern set (symbol/ISO/word prefixes+suffixes, magnitude, ranges, qualifiers, gated accounting negatives, opt-in implicit context) over a text string, Australian false-positive blocking, exact `Decimal` values. Offsets index the string handed in | Read/write parquet; rewrite text; classify columns |
| `process/money_vocab.py` | Money vocabulary tables (currency tiers + full ISO 4217, symbols, currency words, scale table, false-positive regexes, header money/veto terms, null markers). Data only | Execute over documents |
