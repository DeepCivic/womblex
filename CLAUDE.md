...
| `process/renderer.py` | `render_elements()` — project a segment's element run into reviewer-facing markdown, and `rendered_order()` — the `order` sequence as it appears in the output, the handle a reading-order check compares against the source. Narrative slice: TEXT_KINDS render verbatim, one blank line between elements, reusing `chunker.reassemble_narrative` so a reviewed unit and a chunked one share one coordinate space. Table and form rendering land next | Read/write parquet; reorder, deduplicate or tidy; invent content beyond the closed structural set; compare against ground truth (that is the benchmark's suite) |
