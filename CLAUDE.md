...
| `store/build_info.py` | Which build is running: `BuildInfo` (package version + source commit) and `resolve_commit()` — the work tree first, a build-time `_build_stamp.py` second, `unavailable:<reason>` where neither answers. Cached per process | Decide the version (that is `__init__`); write parquet or know about runs |
| `store/run_stamp.py` | Which run produced a file: `RunStamp` (run id, version, commit, config digest, stage) and its `womblex.*` footer keys, plus the loaded-model key read from `utils/models.py` at footer time (a stamp is declared before any model loads); `config_digest` over the *validated* config; `stamp_for_sidecar` / `stamp_from_footers` — the run a downstream sidecar inherits from the extraction shard beside it, since a stage is handed a shard directory and not a run | Decide the run id (the run entry points do); stamp anything that is not a run (`womblex extract <file>`, the register and records ingests) |
...
| `cli/cloud.py` | `enqueue` / `enqueue-stages` / `worker` / `jobs` / `finalize` / `run-stage` CLI — distributed counterpart to `womblex run` over a shared `--store` object-store URI + Postgres `--dsn`. `finalize` consolidates a distributed run's shard manifests into `<run>/manifest.parquet` in the store (the explicit end-step `cmd_run` does locally); `run-stage` generalises that shape to the downstream per-batch stages (`--store`/`--run-id`/`--output-prefix`, mutually exclusive with `--shards`); `enqueue-stages` queues that same sequence for the workers instead, ordered by `pipeline_order` and config-gated (never `pii` / `quality`) | Implement queue/worker/storage/contract logic (those are `cloud/` + `store/remote.py`); run a stage in-process (`enqueue-stages` writes rows only) |
...
`docs/accuracy/*.md` are written by suites in the **womblex-benchmark**
repository (`accuracy/`), which hold the womblex-collection ground truth and
publish here through `accuracy/_paths.py`. Do not edit them by hand, and do not
add a suite here that writes one — run the benchmark's suites to regenerate.
`tests/test_repo_boundaries.py` enforces that.
