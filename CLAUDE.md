...
| `analyse/enrich_stage.py` | `enrich_shards()` over a shard dir — reassemble narrative via `reassemble_narrative`, call Kanon-2 per doc, write `*.enrichment_entities.parquet` + `*.enrichment_meta.parquet` + `*.graph_edges.parquet` siblings (the shippable graph; gains mention→chunk edges when a chunks sidecar exists). With `persist_document`, also writes the raw ILGS Document to `*.enrichment_doc.parquet` for AI-chunking reuse. Per-stage `CheckpointManager`, per-doc failure isolation | Implement matching/linking; know about registers |
