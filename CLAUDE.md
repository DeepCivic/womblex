...
| `utils/models.py` | Resolve local model paths before falling back to downloads; record what a run resolved (`loaded_models()`, byte digests recomputable from the model files alone) for the footer stamp — a probe opts out with `record=False` | Load models (callers do that) |
