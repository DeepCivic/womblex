...
| `process/spellfix_stage.py` | `spellfix_shards()` over a shard dir — read `*.chunks.parquet`, repair text, write `*.chunks_repaired.parquet` (repaired layer) + `*.spellfix_corrections.parquet` (audit). Raw chunks untouched. Per-stage `CheckpointManager` | Implement the corrector (that's `process/spellfix.py`) |
| `store/spellfix_output.py` | `*.chunks_repaired.parquet` (reuses chunk schema) + `*.spellfix_corrections.parquet` schemas + IO (self-contained) | Implement repair/detection |
