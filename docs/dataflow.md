...
3. extract_text(path, profile) → list[ExtractionResult]  (single-element list per source)
      ...
      └── Spreadsheet path → one ExtractionResult per workbook
            ...
            └── kind='sheet_cell' element per non-empty cell — plus every cell a
                merge covers (blank, part of the merge's footprint) — carrying
                value, value_type, formula, number_format, merge_range (the
                merge address on the anchor cell only)
